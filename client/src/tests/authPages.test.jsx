...
describe('authentication pages', () => {
  ...
  it('submits the backend-supported login payload and redirects by role', async () => {
    ...
    authValue.login.mockResolvedValue({
      role: 'JOB_SEEKER',
    })
    ...
    renderWithAuth(
      <Routes>
        ...
        <Route
          ...
          element={<h1>Job seeker dashboard</h1>}
        />
      </Routes>,
    )
  ...
  it('submits only backend-supported job-seeker registration fields', async () => {
    ...
    authApi.registerJobSeeker.mockResolvedValue({
      user: {
    ...
    renderWithAuth(
      <RegistrationPage accountType="job-seeker" />,
    )
  ...
  it('uses the dedicated recruiter registration endpoint', async () => {
    ...
    renderWithAuth(
      <RegistrationPage accountType="recruiter" />,
    )
  ...
  it('maps backend registration field errors separately from the general conflict error', async () => {
    ...
    expect(
      screen.getByLabelText(/Email address/i),
    ).toHaveValue('person@example.com')
  })
})